...
to ``quantforge.X`` so ``from quantforge.core import run_backtest``
...
# with ``_aurora`` because then ``from quantforge.core import X`` would
...
# Eagerly mirror the most common submodules so ``from quantforge.X import Y``
...
for _sub in (
    ...
    try:
        ...
        sys.modules[f"quantforge.{_sub}"] = _mod
    except ImportError:
...
def __getattr__(name: str):  # pragma: no cover - exercised via import paths
    ...
    ``import quantforge.<name>`` lookups skip the dispatch.
    """
    try:
        mod = importlib.import_module(f"aurora.{name}")
    except ImportError as exc:
        raise AttributeError(name) from exc
    sys.modules[f"quantforge.{name}"] = mod
    globals()[name] = mod
    return mod
