...
from aurora.infra.sp500_megarun.microstructure_feature_smoke import (
...
def main() -> int:
    require_github_only_execution(
        "SP500_MEGARUN_MICROSTRUCTURE_FEATURE_SMOKE_F071_F080"
    )
    parser = argparse.ArgumentParser(
        description="Run the train-only technical feature smoke for F071-F080."
    )
    parser.add_argument("--train-snapshot", type=Path, required=True)
    parser.add_argument("--output-dir", type=Path, required=True)
    args = parser.parse_args()
    report = build_microstructure_feature_smoke(
        args.train_snapshot,
