...
CAMPAIGN_ID = "swr_cashfloor_controller_mdd15_corr95_360jobs"
    ...
    for start in range(n):
        ...
        for i in range(start, n):
            ...
            if cushion <= 0.0:
                ...
                safe_scale = cushion / (peak * safe_buffer)
                if safe_scale > safe_leverage:
                    safe_scale = safe_leverage
                if safe_scale < 0.0:
                    safe_scale = 0.0
            ...
            if current_dd <= de_risk_drawdown:
                safe_scale *= de_risk_fraction
                state_scale *= de_risk_fraction
            r = safe_ret[i] * safe_scale + risky_ret[i] * state_scale
            if r < -0.98:
                path_failed = True
                break
            cap *= 1.0 + r
...
def run_shard(output_dir: Path, family_id: int, shard_id: int, configs_per_shard: int, time_budget_minutes: float, top_per_shard: int) -> None:
    ...
    eval_floor_controller_all_starts(np.zeros(130, dtype=np.float64), np.zeros(130, dtype=np.float64), 60, 1.0, 0.1, 0.1, 1.0, -0.05, 0.5)
    ...
    for config_index in range(configs_per_shard):
        ...
        rows.append(
            {
                "strategy_id": f"swr_cashfloor_controller_mdd15_corr95_f{family_id:02d}_s{shard_id:02d}_{config_hash[:16]}",
                "family_id": family_id,
                "shard_id": shard_id,
                ...
                "safe_buffer": float(params["safe_buffer"]),
...
def sample_params(rng: np.random.Generator, family_id: int, assets: list[str]) -> dict[str, Any]:
    return {
        ...
        "safe_buffer": float(rng.choice([0.02, 0.03, 0.05, 0.075, 0.10, 0.15, 0.20])),
...
def run_merge(output_dir: Path) -> None:
    ...
    verified.to_csv(output_dir / "verified_cashfloor_controller_mdd15_report_only.csv", index=False)
    ...
    summary = {
        ...
        "strategy_type": "monthly_cashfloor_controller",
