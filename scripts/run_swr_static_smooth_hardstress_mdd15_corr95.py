...
CAMPAIGN_ID = "swr_static_smooth_hardstress_mdd15_corr95_360jobs"
...
def run_shard(output_dir: Path, family_id: int, shard_id: int, configs_per_shard: int, time_budget_minutes: float, top_per_shard: int) -> None:
    ...
    for config_index in range(configs_per_shard):
        ...
        weights = build_weights(rng, params, assets)
        port = (returns[assets] * pd.Series(weights, index=assets)).sum(axis=1)
        if port.min() <= -0.98:
            continue
        ...
        config_hash = hashlib.sha256(json.dumps({"params": params, "weights": dict(zip(assets, weights, strict=False))}, sort_keys=True).encode("utf-8")).hexdigest()
        rows.append(
            {
                "strategy_id": f"swr_static_smooth_hardstress_mdd15_corr95_f{family_id:02d}_s{shard_id:02d}_{config_hash[:16]}",
                "family_id": family_id,
                "shard_id": shard_id,
                ...
                "train_stress_windows": json.dumps([[str(start.date()), str(end.date())] for start, end in TRAIN_STRESS_WINDOWS]),
                "params_json": json.dumps(params, sort_keys=True),
                "weights_json": json.dumps({asset: float(weight) for asset, weight in zip(assets, weights, strict=False)}, sort_keys=True),
...
def sample_params(rng: np.random.Generator, family_id: int, assets: list[str]) -> dict[str, Any]:
    return {
        "family_id": family_id,
        "gross": float(rng.uniform(0.5, 16.0 if family_id >= 8 else 8.0)),
        "sparsity": int(rng.choice([2, 3, 4, 5, 6, len(assets)])),
        "long_short_mode": str(rng.choice(["long_only", "long_short", "market_neutral", "short_bias"])),
        "inverse_bias": float(rng.uniform(0.0, 3.0)),
        "treasury_bias": float(rng.uniform(0.0, 3.0)),
        "equity_bias": float(rng.uniform(0.0, 3.0)),
        "sector_bias": float(rng.uniform(0.0, 3.0)),
        "concentration_power": float(rng.choice([0.5, 1.0, 1.5, 2.0])),
    }


def build_weights(rng: np.random.Generator, params: dict[str, Any], assets: list[str]) -> np.ndarray:
    raw = rng.normal(0.0, 1.0, len(assets))
    bias = np.ones(len(assets))
    for i, asset in enumerate(assets):
        if asset == "inverse_sp500":
            bias[i] += float(params["inverse_bias"])
        elif "treasury" in asset:
            bias[i] += float(params["treasury_bias"])
        elif asset in {"ndx", "sp500", "small"}:
            bias[i] += float(params["equity_bias"])
        elif asset in {"energy", "financial"}:
            bias[i] += float(params["sector_bias"])
    raw *= bias
    mode = params["long_short_mode"]
    if mode == "long_only":
        raw = np.abs(raw)
    elif mode == "market_neutral":
        raw = raw - raw.mean()
    elif mode == "short_bias":
        raw = raw - abs(raw.mean())
    k = min(int(params["sparsity"]), len(raw))
    keep = np.argsort(np.abs(raw))[-k:]
    sparse = np.zeros_like(raw)
    sparse[keep] = raw[keep]
    signed = np.sign(sparse) * np.abs(sparse) ** float(params["concentration_power"])
    gross = np.abs(signed).sum()
    if gross <= 0.0:
        return np.zeros_like(raw)
    return signed / gross * float(params["gross"])
...
def run_merge(output_dir: Path) -> None:
    ...
    verified.to_csv(output_dir / "verified_static_smooth_mdd15_report_only.csv", index=False)
    ...
    summary = {
        ...
        "train_stress_windows": [[str(start.date()), str(end.date())] for start, end in TRAIN_STRESS_WINDOWS],
