...
from aurora.infra.sp500_megarun.characteristic_feature_smoke import (
...
def main() -> int:
    require_github_only_execution(
        "SP500_MEGARUN_CHARACTERISTIC_FEATURE_SMOKE_F151_F160"
    )
    parser = argparse.ArgumentParser(
        description="Run train-only characteristic feature smoke F151-F160."
    )
    parser.add_argument("--train-snapshot", type=Path, required=True)
    parser.add_argument("--output-dir", type=Path, required=True)
    args = parser.parse_args()
    report = build_characteristic_feature_smoke(
        args.train_snapshot,
