...
from aurora.research.search_quality import (  # noqa: E402
    SearchQualityConfig,
    SearchQualityState,
    early_prune_reason,
...
def _spy_momentum_trend_dataset(
    ...
    *,
    ...
    if require_feature_data_from_year is not None:
        availability = train_x.attrs.get("availability_mask")
        if not isinstance(availability, pd.DataFrame):
            availability = train_x.notna()
        kept, history_rejected = filter_features_by_history(
            tuple(train_x.columns),
            pd.DataFrame(availability),
            start_year=int(require_feature_data_from_year),
            min_weeks_per_year=int(min_feature_weeks_per_year),
        )
        if not kept:
            raise ValueError("no SPY momentum/trend features have enough history from requested year")
        train_x = train_x.loc[:, kept].copy()
        valid_x = valid_x.loc[:, kept].copy()
        train_x.attrs["availability_mask"] = pd.DataFrame(availability).loc[:, kept]
        valid_availability = valid_attrs.get("availability_mask")
        if isinstance(valid_availability, pd.DataFrame):
            valid_x.attrs["availability_mask"] = valid_availability.loc[:, kept]
