...
CAMPAIGN_ID = "swr_path_cppi_focused_cv_mdd15_trainonly_corr95_360jobs"
...
def run_shard(output_dir: Path, family_id: int, shard_id: int, configs_per_shard: int, time_budget_minutes: float, top_per_shard: int) -> None:
    ...
    for config_index in range(configs_per_shard):
        ...
        rows.append(
            {
                "strategy_id": f"swr_path_cppi_focused_cv_mdd15_corr95_f{family_id:02d}_s{shard_id:02d}_{config_hash[:16]}",
                "family_id": family_id,
                "shard_id": shard_id,
...
def sample_params(rng: np.random.Generator, family_id: int) -> dict[str, Any]:
    focused = family_id in {0, 1, 2, 3, 4, 5, 10, 11, 12, 13}
    base_floor = float(rng.choice([0.80, 0.84, 0.87, 0.90, 0.93, 0.96]))
    risk_modes = ["sp500", "inverse_sp500", "inverse_blend", "risk_blend", "risk_vs_treasury", "multi_tsmom"]
    if not focused:
        risk_modes += ["ndx", "sector_blend", "sector_vs_treasury"]
    return {
        "family_id": family_id,
        "risk_mode": str(rng.choice(risk_modes)),
        "safe_mode": str(rng.choice(["shy", "ief", "safe_blend", "inverse_sp500", "tlt_tsmom", "ief_tsmom", "safe_tsmom_blend"])),
        "lookback": int(rng.choice([1, 1, 3, 3, 6])),
        "vol_lookback": int(rng.choice([3, 6, 12])),
        "top_n": int(rng.choice([1, 2, 3])),
        "bottom_n": int(rng.choice([0, 1, 2])),
        "score_power": float(rng.choice([0.5, 1.0, 1.5, 2.0])),
        "base_exposure": float(rng.uniform(0.0, 2.8 if focused else 4.5)),
        "multiplier": float(rng.uniform(2.0, 24.0 if focused else 34.0)),
        "floor_pct": base_floor + float(rng.uniform(-0.015, 0.015)),
        "max_risk_exposure": float(rng.uniform(0.20, 3.5 if focused else 7.0)),
        "safe_exposure": float(rng.uniform(4.0, 18.0 if focused else 24.0)),
        "max_gross": float(rng.uniform(4.0, 18.0 if focused else 26.0)),
        "allow_short": int(rng.integers(0, 2)),
        "signal_threshold": float(rng.uniform(-0.055, 0.035)),
        "guard_threshold": float(rng.choice([-0.12, -0.10, -0.08, -0.06, -0.04, -0.03])),
        "guard_scale": float(rng.choice([0.0, 0.03, 0.08, 0.15, 0.25, 0.40, 0.60])),
        "recovery_boost": float(rng.choice([0.0, 0.25, 0.50, 1.0, 1.5])),
        "risk_blend_ndx": float(rng.uniform(-0.8, 1.2)),
        "risk_blend_sp500": float(rng.uniform(-0.8, 1.2)),
        "risk_blend_tlt": float(rng.uniform(-0.2, 2.2)),
        "risk_blend_energy": float(rng.uniform(-0.2, 1.4)),
        "risk_blend_financial": float(rng.uniform(-0.2, 1.4)),
        "risk_blend_inverse": float(rng.uniform(0.0, 2.8)),
        "safe_blend_shy": float(rng.uniform(0.2, 2.2)),
        "safe_blend_ief": float(rng.uniform(-0.2, 1.8)),
        "safe_blend_tlt": float(rng.uniform(-0.2, 2.4)),
    }


def anchor_params(family_id: int, shard_id: int, config_index: int) -> dict[str, Any]:
    i = family_id * 1009 + shard_id * 37 + config_index
    floors = [0.82, 0.85, 0.88, 0.91, 0.94, 0.965]
    max_risk = [0.35, 0.55, 0.80, 1.10, 1.45, 1.80, 2.20]
    safe = [8.0, 10.0, 12.0, 14.0, 16.5, 19.0]
    max_gross = [7.0, 9.0, 11.0, 13.0, 16.0, 19.0]
    risk_modes = ["sp500", "inverse_sp500", "inverse_blend", "risk_blend", "risk_vs_treasury", "multi_tsmom"]
    safe_modes = ["shy", "ief", "safe_blend", "inverse_sp500", "safe_tsmom_blend"]
    return {
        "family_id": family_id,
        "risk_mode": risk_modes[i % len(risk_modes)],
        "safe_mode": safe_modes[(i // 2) % len(safe_modes)],
        "lookback": int([1, 1, 3, 3, 6][i % 5]),
        "vol_lookback": int([3, 6, 12][i % 3]),
        "top_n": int([1, 2, 3][i % 3]),
        "bottom_n": int([0, 1, 2][i % 3]),
        "score_power": float([0.5, 1.0, 1.5, 2.0][i % 4]),
        "base_exposure": float([0.0, 0.25, 0.50, 0.90, 1.35, 1.80][i % 6]),
        "multiplier": float([4.0, 7.0, 10.0, 14.0, 18.0, 22.0, 27.0][i % 7]),
        "floor_pct": float(floors[(i // 3) % len(floors)]),
        "max_risk_exposure": float(max_risk[(i // 5) % len(max_risk)]),
        "safe_exposure": float(safe[(i // 7) % len(safe)]),
        "max_gross": float(max_gross[(i // 11) % len(max_gross)]),
        "allow_short": int([0, 0, 1][i % 3]),
        "signal_threshold": float([-0.055, -0.04, -0.025, -0.01, 0.0, 0.02][i % 6]),
        "guard_threshold": float([-0.12, -0.10, -0.08, -0.06, -0.04, -0.03][(i // 13) % 6]),
        "guard_scale": float([0.0, 0.03, 0.08, 0.15, 0.25, 0.40][i % 6]),
        "recovery_boost": float([0.0, 0.25, 0.5, 1.0, 1.5][(i // 17) % 5]),
        "risk_blend_ndx": float([0.0, 0.3, 0.7, 1.0][i % 4]),
        "risk_blend_sp500": float([-0.3, 0.0, 0.4, 0.8][(i // 2) % 4]),
        "risk_blend_tlt": float([0.2, 0.7, 1.2, 1.8][(i // 3) % 4]),
        "risk_blend_energy": float([0.0, 0.3, 0.8, 1.2][(i // 5) % 4]),
        "risk_blend_financial": float([0.0, 0.3, 0.8, 1.2][(i // 7) % 4]),
        "risk_blend_inverse": float([0.5, 1.0, 1.5, 2.2, 2.8][(i // 11) % 5]),
        "safe_blend_shy": float([0.6, 1.0, 1.4, 2.0][i % 4]),
        "safe_blend_ief": float([0.0, 0.4, 0.8, 1.4][(i // 2) % 4]),
        "safe_blend_tlt": float([0.0, 0.5, 1.0, 1.8, 2.4][(i // 3) % 5]),
    }
...
def run_merge(output_dir: Path) -> None:
    ...
    verified.to_csv(output_dir / "verified_path_cppi_focused_cv_mdd15_report_only.csv", index=False)
