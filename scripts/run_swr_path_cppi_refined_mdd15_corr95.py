...
CAMPAIGN_ID = "swr_path_cppi_refined_mdd15_trainonly_corr95_360jobs"
...
def run_shard(output_dir: Path, family_id: int, shard_id: int, configs_per_shard: int, time_budget_minutes: float, top_per_shard: int) -> None:
    ...
    for config_index in range(configs_per_shard):
        ...
        rows.append(
            {
                "strategy_id": f"swr_path_cppi_refined_mdd15_corr95_f{family_id:02d}_s{shard_id:02d}_{config_hash[:16]}",
                "family_id": family_id,
                "shard_id": shard_id,
...
def sample_params(rng: np.random.Generator, family_id: int) -> dict[str, Any]:
    ...
    return {
        "family_id": family_id,
        "risk_mode": str(rng.choice(["sp500", "ndx", "risk_vs_treasury", "inverse_blend", "risk_blend", "multi_tsmom"], p=[0.24, 0.16, 0.18, 0.18, 0.14, 0.10])),
...
def run_merge(output_dir: Path) -> None:
    ...
    verified.to_csv(output_dir / "verified_path_cppi_refined_mdd15_report_only.csv", index=False)
