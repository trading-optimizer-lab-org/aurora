...
from aurora.infra.sp500_megarun.nonlinear_feature_smoke import (
...
def main() -> int:
    require_github_only_execution(
        "SP500_MEGARUN_NONLINEAR_FEATURE_SMOKE_F131_F140"
    )
    parser = argparse.ArgumentParser(
        description="Run train-only feature smoke F131-F140."
    )
    parser.add_argument("--train-snapshot", type=Path, required=True)
    parser.add_argument("--output-dir", type=Path, required=True)
    args = parser.parse_args()
    report = build_nonlinear_feature_smoke(
