...
SOURCE_METADATA = {
    ...
    for item in (
        ...
        _meta(
            ...
            start="two rolling years",
            frequency="end-of-day",
            lag="previous session",
            identifiers="ticker,OCC option symbol",
            available_at="aggregate timestamp",
            pit="two years of OHLCV aggregates; no historical IV or open interest",
            redistribution="individual personal non-business use only",
        ),
        _meta(
            ...
            start="contract lifetime when a valid OCC option symbol is already known",
            ...
            pit="OHLCV by known contract; no historical chain, IV, open interest or surface",
        ...
        _meta(
            "alpha_vantage_options_premium",
            "Alpha Vantage",
            "https://www.alphavantage.co/documentation/",
            "https://www.alphavantage.co/terms_of_service/",
            account=True,
            card=True,
            auth="premium API key",
            rate="premium plan dependent",
            start="2008-01-01",
            frequency="daily historical chain query",
            lag="endpoint dependent",
            identifiers="ticker,OCC option contract",
            available_at="requested historical trading date",
            pit="15+ years of historical chains but no OptionMetrics-equivalent full sample",
            redistribution="premium personal use; commercial use requires sales agreement",
        ),
        _meta(
...
def _route_source_ids(signal: str, row: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    if signal in OPTION_SIGNALS:
        sources += [
            ...
            "alpha_vantage_options_premium", "optionmetrics_ivydb_us", "wrds_linking_suite",
        ]
...
def render_research_report(
    ...
    lines += [
        ...
        "- [Tradier](https://docs.tradier.com/reference/brokerage-api-markets-get-history) documenta OHLCV histórico cuando ya se conoce el símbolo OCC, pero no ofrece enumeración histórica de cadenas, IV, open interest, superficies ni identidad permanente. Su acceso autoservicio es solo personal salvo aprobación como Partner.",
        "- [Massive Options Basic](https://massive.com/options) ofrece dos años de OHLCV al individuo, sin IV ni open interest históricos y con condiciones no empresariales. [Alpha Vantage Historical Options](https://www.alphavantage.co/documentation/) es premium y empieza en 2008. Ninguna ruta cubre gratis y con autorización las muestras OpenAP iniciadas en 1996.",
