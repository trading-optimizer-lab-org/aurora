...
def test_completion_audit_can_consume_patent_source_evidence():
    ...
    assert (
        "needs: [financing_issuance_probe, valuation_accounting_probe, accruals_noa_probe, complex_accounting_probe, rio_probe, microstructure_probe, relationship_probe, analyst_probe, patent_probe, short_interest_probe, options_probe]"
        in text
    )
    assert "PATENT_EVIDENCE_RUN_ID" in text
