...
TARGET_DEPENDENCIES = {
    "AM": ("assets",),
    "BM": ("equity",),
    "CashProd": ("assets", "cash"),
    "CF": ("net_income", "depreciation"),
    "cfp": ("operating_cash_flow",),
    "EP": ("net_income",),
    "Leverage": ("liabilities",),
    "NetDebtPrice": ("debt_current", "debt_long", "cash"),
    "NetPayoutYield": ("dividends", "repurchases", "share_issuance"),
    "PayoutYield": ("dividends", "repurchases"),
    "RD": ("rd",),
    "SP": ("revenue",),
    "AdExp": ("advertising",),
}
...
def _feature_frame() -> pd.DataFrame:
    fillers = [f"Filler{index:03d}" for index in range(172)]
    signals = list(RECOVERED_CURRENT_FEATURE_TARGETS) + fillers
    rows = []
    for symbol in ("AAA", "BBB"):
        for signal_index, signal in enumerate(signals):
            is_target = signal in TARGET_DEPENDENCIES
            rows.append(
                {
                    "as_of": "2026-08-08",
                    "symbol": symbol,
...
def _concept_inputs() -> pd.DataFrame:
    concepts = sorted(
        {concept for values in TARGET_DEPENDENCIES.values() for concept in values}
    )
    rows = []
