...
def sign_canary_request(private, *, definition_sha256="c" * 64, generation=10):
...
def _public_scope(context_sha256="a" * 64, request_sha256="b" * 64):
    identity = dict(campaign_key="catalog-fast-canary-v1", generation=10,
...
def test_signed_request_cannot_be_replaced_by_rehashed_cli_fields(tmp_path, monkeypatch, mutation):
    ...
    request = sign_canary_request(private, generation=9 if mutation == "generation" else 10)
    payload = request.model_dump(mode="json")
    if mutation == "signature":
        payload["requester_attestation_b64"] = b64encode(bytes(256)).decode("ascii")
    if mutation == "generation":
        payload["launch_generation"] = 10
...
@pytest.mark.parametrize("generation", (2, 3, 4, 5, 6, 7, 8, 9, 10))
def test_actual_workflow_bootstrap_needs_only_stdlib(tmp_path, generation):
    ...
    assert values["fault_enabled"] == ("true" if generation == 10 else "false")
    if generation == 10:
        assert values["acceptance_token"] == canary.build_canary_acceptance_token(
            campaign_key="catalog-fast-canary-v1", generation=10,
