...
def _build_plan(**overrides: Unpack[_PlanOverrides]) -> CatalogGlobalReuseExecutionPlanV1:
    from scripts.plan_sp500_optimized_catalog_run import (
...
def test_all_cached_recipes_emit_no_recipe_workers_or_recipe_bundles() -> None:
    all_ids = tuple(f"strategy-{ordinal:03d}" for ordinal in range(5))
    plan = _build_plan(cached_strategy_ids=all_ids)
...
def test_partial_recipe_plan_seals_and_verifies_coherent_work_inputs(tmp_path) -> None:
    ...
    contract_payload["workload"] = {
        **workload,
        ...
        "canonical_recipes": 5,
        "unique_components": 4,
        "expected_new_recipes": 3,
        "expected_prior_cache_hits": 2,
