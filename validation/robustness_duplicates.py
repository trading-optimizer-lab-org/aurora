...
def assign_duplicate_groups(
    ...
    if "exposure" in returns.columns and len(candidate_ids) > 1:
        ...
        max_active_pct = float(config.position_jaccard_max_active_pct)
        active_pct = active.mean(axis=0).fillna(0.0)
        for i, left in enumerate(candidate_ids):
            for right in candidate_ids[i + 1 :]:
                if active_pct[left] >= max_active_pct and active_pct[right] >= max_active_pct:
                    continue
