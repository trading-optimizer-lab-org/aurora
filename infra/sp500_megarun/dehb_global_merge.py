...
def register_evaluation_result_hash(
    ...
    """Register one key/result identity and reject conflicting science."""

    cache_key = str(cache_key_sha256)
    result_hash = str(result_sha256)
    if len(cache_key) != 64 or len(result_hash) != 64:
        raise GlobalMergeError("TRIAL_CACHE_HASH_INVALID")
    existing_result_hash = registry.get(cache_key)
    if existing_result_hash is not None and existing_result_hash != result_hash:
        raise GlobalMergeError(f"GLOBAL_EVALUATION_CACHE_CONFLICT:{cache_key}")
    registry.setdefault(cache_key, result_hash)
...
def candidate_records_equivalent(left: Mapping[str, Any], right: Mapping[str, Any]) -> bool:
    ...
    if not isinstance(left_archive, Sequence) or isinstance(left_archive, (str, bytes)):
        return False
    if not isinstance(right_archive, Sequence) or isinstance(right_archive, (str, bytes)):
        return False
...
def compare_prefix_feature_frames(
    ...
    *,
    ...
    def normalize(frame: pd.DataFrame) -> pd.DataFrame:
        ...
        if missing:
            raise GlobalMergeError(f"PREFIX_FEATURE_COLUMN_MISSING:{','.join(missing)}")
...
def review_candidate_prefix_invariance(
    ...
    *,
    ...
    hot_cache = compare_prefix_feature_frames(full, hot, cutoff=str(contract.search_end))
    cold_cache = compare_prefix_feature_frames(full, cold, cutoff=str(contract.search_end))
    ...
    for cutoff in cutoffs:
        ...
        if expected_count == 0:
            ...
            continue
        try:
            prefix = TrainLaneEvaluator(**common, maximum_date=cutoff)(lane_id, configuration)
    ...
    cache_reproduction_passed = hot_cache["passed"] is True and cold_cache["passed"] is True
    return {
        "schema_version": 1,
        "lane_id": lane_id,
...
def collect_verified_campaign_inventory(
    ...
    *,
    ...
    expected = {
        island.island_id for job in build_island_schedule(contract) for island in job.islands
    }
    ...
    full_fidelity_physical_evaluations = 0
    cache_hits_by_origin: dict[str, int] = {}
    physical_runtime_seconds = 0.0
    estimated_runtime_seconds_saved = 0.0
    evaluation_result_hashes: dict[str, str] = {}
    determinism_audit_physical_evaluations = 0
    ...
    for island_id in sorted(by_island):
        ...
        if manifest.get("determinism_audit_passed") is not True:
            raise GlobalMergeError("ISLAND_DETERMINISM_AUDIT_MISSING")
        determinism_audit_physical_evaluations += int(
            manifest.get("determinism_audit_physical_evaluations", 0)
        )
        ...
        required_cache_columns = {"evaluation_origin", "physical_runtime_seconds", "info_json"}
        if not required_cache_columns <= set(trial_frame.columns):
            raise GlobalMergeError("TRIAL_CACHE_EVIDENCE_MISSING")
        for trial in trial_frame.to_dict(orient="records"):
            origin = str(trial["evaluation_origin"])
            register_evaluation_result_hash(
                evaluation_result_hashes,
                cache_key_sha256=trial.get("cache_key_sha256"),
                result_sha256=trial.get("cache_result_sha256"),
            )
            try:
                trial_info = json.loads(str(trial["info_json"]))
            except (json.JSONDecodeError, TypeError) as exc:
                raise GlobalMergeError("TRIAL_INFO_JSON_INVALID") from exc
            if origin == "physical":
                physical_evaluations += 1
                physical_runtime_seconds += float(trial.get("physical_runtime_seconds", 0.0))
                if trial_info.get("full_fidelity") is True:
                    full_fidelity_physical_evaluations += 1
            else:
                cache_hits_by_origin[origin] = cache_hits_by_origin.get(origin, 0) + 1
                estimated_runtime_seconds_saved += float(
                    trial_info.get("objective_runtime_seconds", 0.0)
                )
    ...
    inventory = {
        ...
        "cache_hits": sum(cache_hits_by_origin.values()),
        "cache_hits_by_origin": dict(sorted(cache_hits_by_origin.items())),
        "full_fidelity_cache_hits": (full_fidelity_rows - full_fidelity_physical_evaluations),
        "physical_runtime_seconds": physical_runtime_seconds,
        "estimated_runtime_seconds_saved": estimated_runtime_seconds_saved,
        "cache_conflicts": 0,
        "unique_evaluation_keys": len(evaluation_result_hashes),
        "determinism_audit_physical_evaluations": (determinism_audit_physical_evaluations),
        "total_physical_computations": (
            physical_evaluations + determinism_audit_physical_evaluations
        ),
        "unique_strategies": len(candidates),
...
def reconstruct_candidate_returns(
    ...
    *,
    ...
    for record in candidate_records:
        ...
        if identity != candidate_id or position != record.get("position_fingerprint"):
