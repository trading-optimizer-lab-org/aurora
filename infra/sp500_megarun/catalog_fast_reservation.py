...
def load_owner_terminal_receipt(
    *, client: _OwnerReader, owner: FastGateOwnerEvidence, issue_number: int,
    ...
    if expected_terminal_sha256 is not None and receipt.receipt_sha256 != expected_terminal_sha256:
        if (issue_number != 378 or owner.run_id != 36032882147
            ...
            or receipt.receipt_sha256 != "b8918eee4510d094153756c02baa387c61d5623f91335d5f34707d9e5493ac7f"
            or receipt.reason_code != "ATLAS_TERMINAL_CATALOG_HASH_INVALID"):
            raise ValueError("CATALOG_FAST_AUTHORITY_TERMINAL_CONFLICT")
        return _load_atlas_terminal_correction(
            client=client, owner=owner, expected_sha256=expected_terminal_sha256,
            download_archive=download_archive,
        )
    return receipt


def _load_atlas_terminal_correction(*, client: _OwnerReader, owner: FastGateOwnerEvidence,
                                    expected_sha256: str,
                                    download_archive: Callable[[int], bytes]) -> CatalogTerminalReceipt:
    """Resolve only the authority-pinned, protected correction of Atlas issue 378."""
