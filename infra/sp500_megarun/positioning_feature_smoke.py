"""Train-only GitHub smoke for executable SP500 lanes F081-F090."""
...
from aurora.infra.sp500_megarun.positioning_feature_engine import (
    evaluate_positioning_family_batch,
    evaluate_positioning_lane,
)


class PositioningFeatureSmokeError(ValueError):
...
_EXECUTABLE_LANES = tuple(f"F{index:03d}" for index in range(81, 91))
...
def _repair_positioning_configuration(
    lane_id: str,
    parameter: str,
    configuration: dict[str, Any],
) -> dict[str, Any]:
    if lane_id == "F082" and parameter == "lag":
        configuration["statistic"] = "change"
    if lane_id == "F083" and parameter == "lag":
        configuration["statistic"] = "short_pressure"
    if lane_id == "F085" and parameter == "window":
        configuration["statistic"] = "persistence"
    if lane_id == "F086" and parameter in {"window", "lag"}:
        configuration["statistic"] = "change"
    if lane_id == "F087" and parameter in {"window", "lag"}:
        configuration["statistic"] = "gap_change"
    if lane_id == "F088" and parameter == "lag":
        configuration["statistic"] = "change"
    return configuration


def build_positioning_feature_smoke(
    train_snapshot: str | Path,
    *,
    output_dir: str | Path,
) -> dict[str, Any]:
    """Normalize, execute and audit F081-F090 without mounting validation."""

    snapshot = Path(train_snapshot)
    if snapshot.name != _TRAIN_PARTITION:
        raise PositioningFeatureSmokeError("TRAIN_PARTITION_REQUIRED")
    datasets: dict[str, pd.DataFrame] = {}
    for dataset_id in _REQUIRED_DATASETS:
        target = snapshot / f"{dataset_id}.parquet"
        if not target.is_file():
            raise PositioningFeatureSmokeError(
    ...
    panels = {
        ...
        "finra_margin": normalize_finra_margin_panel(
            datasets["D_FINRA_MARGIN"], sessions=sessions
        ),
        "margin": normalize_finra_margin_panel(
            datasets["D_MARGIN"], sessions=sessions
        ),
        "legacy_cftc": normalize_cftc_sp500_panel(
        ...
        "industries": normalize_french_industry_panel(
            datasets["D_FRENCH_INDUSTRIES"], sessions=sessions
        ),
    }
    outputs = dict(evaluate_positioning_family_batch(panels))
    ...
    parameter_audit = audit_frozen_parameter_choices(
        ...
        evaluator=lambda lane_id, configuration: evaluate_positioning_lane(
            lane_id,
            panels,
            configuration,
        ),
        expected_years=expected_years,
        repair=_repair_positioning_configuration,
    )
    ...
    report: dict[str, Any] = {
        ...
        "scope": "technical_positioning_feature_smoke_train_only",
    ...
    (root / "positioning_feature_smoke_report.json").write_text(
        json.dumps(report, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    (root / "parameter_choice_audit_F081_F090.json").write_text(
