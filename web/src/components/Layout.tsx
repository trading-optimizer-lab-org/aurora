...
const navItems: Array<{ id: ViewName; label: string; icon: string }> = [
  ...
  { id: "artifacts", label: "Archivos", icon: "□" },
  { id: "workflows", label: "Procesos de GitHub", icon: "◈" },
];
...
export function Layout({ view, onNavigate, overview, children, isDemo }: LayoutProps) {
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      <div className="brand"><span className="brand-orbit" /><div><strong>AURORA</strong><small>SEGUIMIENTO</small></div></div>
      ...
      <div className="sidebar-footer">
        <div className="archive-mini"><div className="mini-head"><span>ESPACIO GUARDADO</span><strong>{archivePercent}%</strong></div><div className="mini-track"><span style={{ width: `${archivePercent}%` }} /></div><small>{overview ? `${formatBytes(overview.archive.used_bytes)} / ${formatBytes(overview.archive.quota_bytes)}` : "cargando"}</small></div>
        <div className="read-only"><span className="read-only-dot" /> Solo lectura</div>
      </div>
    </aside>
    <main className="main-content">
      <header className="topbar"><div className="mobile-brand"><span className="brand-orbit" /><strong>AURORA</strong></div><div className="topbar-meta"><span className={`live-indicator ${overview?.totals.active_runs ? "is-live" : ""}`}><i /> {overview?.totals.active_runs ? `${overview.totals.active_runs} en marcha` : "Nada en marcha"}</span><span className="topbar-separator" /><span className="sync-time">Actualizado {overview?.sync.last_success_at ? formatDate(overview.sync.last_success_at) : "—"}</span>{isDemo && <span className="demo-badge">MODO DEMO</span>}</div></header>
