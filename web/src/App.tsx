...
export function App({ client = dashboardClient }: AppProps) {
  ...
  const renderOverview = () => {
    ...
    return <div className="dashboard-page">
      ...
      <div className="metric-grid"><MetricCard label="Runs activos" value={formatCompact(overview.totals.active_runs)} detail="actualizados automáticamente" tone="cyan" icon="↗" /><MetricCard label="Runs indexados" value={formatCompact(overview.totals.runs)} detail={`${formatCompact(overview.totals.workflows)} workflows`} tone="violet" icon="◈" /><MetricCard label="Resultados" value={formatCompact(overview.totals.parsed_results)} detail="métricas interpretadas" tone="green" icon="∿" /><MetricCard label="Archivo" value={`${archivePercent}%`} detail={archiveDetail} tone="orange" icon="□" /></div>
